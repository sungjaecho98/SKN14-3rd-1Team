...
with tab1:
    ...
    with col2:
        if st.button("⚡ 피로 회복에 도움되는 것은?"):
            st.session_state.chat_history.append({
                "type": "user", 
                "message": "피로 회복에 도움이 되는 영양제는 무엇인가요?"
            })
    ...
    if st.button("전송") and user_input:
        ...
        # AI 응답 생성 (실제로는 LLM 호출)
        ai_responses = {
            "피부": "피부 건강을 위해서는 **비타민C**, **비타민E**, **아연**이 포함된 제품을 추천드립니다. 특히 콜라겐과 히알루론산이 함께 포함된 제품이 효과적입니다. 항산화 성분이 풍부한 제품을 선택하시는 것이 좋습니다.",
            "피로": "피로 회복을 위해서는 **비타민B 복합체**, **마그네슘**, **코엔자임Q10**을 추천드립니다. 특히 야근이나 스트레스가 많으신 분들께는 마그네슘과 비타민B12가 함께 포함된 제품이 도움이 됩니다.",
            "비타민D": "비타민D와 칼슘은 **함께 복용하면 좋은 조합**입니다. 비타민D는 칼슘의 흡수를 도와주므로 시너지 효과가 있습니다. 다만 **마그네슘**도 함께 섭취하시면 더욱 효과적입니다.",
            "default": "구체적인 건강 목표나 현재 상황을 알려주시면 더 정확한 추천을 드릴 수 있습니다. 예를 들어 '면역력 강화', '피부 건강', '피로 회복' 등의 키워드를 사용해보세요."
        }
        
        # 키워드 기반 응답 선택
        response = ai_responses["default"]
        for keyword, answer in ai_responses.items():
    ...
    for chat in st.session_state.chat_history:
        if chat["type"] == "user":
            ...
            <div class="chat-message bot-message">
                <strong>🤖 NutriWise AI:</strong> {chat["message"]}
            </div>
            """, unsafe_allow_html=True)
...
2. LLM + RAG 시스템:
   - OpenAI API 또는 Hugging Face 모델 사용
   - 벡터 데이터베이스 (Pinecone, Chroma) 연동
   - 제품 문서 임베딩 및 검색
